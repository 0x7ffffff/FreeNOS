...
public:
    ...
    ChannelServer(Base *inst)
            : m_instance(inst), m_client(ChannelClient::instance()), m_registry(m_client->getRegistry()),
              m_kernelEvent(Channel::Consumer, sizeof(ProcessEvent)), m_ipcHandlers(), m_irqHandlers() {
