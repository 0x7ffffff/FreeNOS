...
ProcessShares::Result ProcessShares::createShare(ProcessShares &instance,
    ...
    if (localMem->findFree(localShare->range.size, MemoryMap::UserShare, &localShare->range.virt) !=
    ...
    if (remoteMem->findFree(remoteShare->range.size, MemoryMap::UserShare, &remoteShare->range.virt) !=
        MemoryContext::Success ||
